...
func (c *Controller) processNextItem() bool {
	...
	if err != nil {
		if c.queue.NumRequeues(key) < 5 {
			fmt.Errorf("Failed processing item with key %s with error %v, retrying1", key, err)
			c.queue.AddRateLimited(key)
		} else {
			fmt.Errorf("Failed processing item with key %s with error %v, no more retries", key, err)
			c.queue.Forget(key)
